...
- [ ] **DCF Hızlı Değerleme** `[M]` `[P3]` — FMP `/stable/discounted-cash-flow`; "Bugünkü adil değer: $145, şu an $132 — %9 ucuz". UI'da "tahmini" etiketi zorunlu. **Sadece US_STOCK** — free tier US hisselerinde çalışıyor (2026-05-21 test: AAPL/MSFT HTTP 200), BIST tickerları HTTP 402 (premium); FMP'nin mevcut US-only kullanımıyla tutarlı. Burst rate limit sorunu yok; günlük kota `fund_cache` haftalık cron refresh ile absorbe edilir.
