...
3. ~~**Alım Fiyatı Bölgesi Analizi (52W Konumu)**~~ `[S→M][P2]` `Sprint-22` `2026-05-19` ✅ kod yazıldı — frontend canlıda (graceful degrade: h_52w/l_52w NULL ise bar gizli), backend deploy bekliyor: (1) `supabase/migrations/021_price_cache_52w.sql` apply (`npx supabase db push` veya Supabase Dashboard), (2) `fetch-prices` + `refresh-price-cache` edge fn'leri deploy. Closes-only 52W high/low (`compute52w` helper, 3 historical fn'e eklendi); price_cache upsert payload'u genişletildi. TickerDetailTab "Giriş Kalitesi" bar: held US_STOCK ve TRY-cinsli BIST için gradient bar (yeşil→sarı→kırmızı) + avg_cost dikey marker + güncel fiyat disk marker + verdict ("düşük bantta/orta/zirveye yakın"). Eşik: cost <33% iyi, 33-66% orta, >66% zirveye yakın.
...
6. **Voice-Add (PWA + Telegram bot)** `[L][P2]` — Sesli işlem ekleme. PWA AddTab'a "Ses" mode + hold-to-record; Telegram bot voice message → inline-keyboard onay → kaydet. Shared `voicePipeline` helper (Whisper + parse-transaction); 3 yeni tablo (telegram_link_codes/telegram_links/telegram_pending) + 2 yeni edge fn (transcribe-audio, telegram-webhook) + 3 yeni secret (OPENAI_API_KEY, TELEGRAM_BOT_TOKEN, TELEGRAM_WEBHOOK_SECRET). Spec: `docs/superpowers/specs/2026-05-17-voice-add-design.md`. CASH/DEPOSIT/BES kapsam dışı.
