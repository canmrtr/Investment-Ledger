...
13 tablo (positions, transactions, splits, profiles, price_cache, portfolios, watchlist, follows, portfolio_activities, fund_cache, adr_bist_map, tefas_funds, feedback) + 4 DB RPC + 2 pg_cron job. Tam tablo yapısı, RLS niyeti, RPC imzaları ve cron → **`SCHEMA.md`**.
...
- **Settings** (hamburger menüden açılır; Sprint 28 kullanıcı-odaklı IA): **Görünür** = Account (`AccountSection`), Portföy (public/share), Görünüm (tema), Veri (yalnız "↻ Şimdi Güncelle"), Araçlar (Export CSV + İşlem Geçmişi), **Geri Bildirim/Destek** (`FeedbackSection` → `feedback` tablosuna RLS-korumalı insert), Çıkış. **Katlı** = "Gelişmiş / Geliştirici" collapsible (`advancedOpen`): Tarihi Veri, TEFAS Katalogu Yenile, 🔍 Bağlantı Test, ♻️ Pozisyonları Yeniden Hesapla, 🔄 Splitleri Senkronize Et, Sistem Durumu (iç içe collapsible). Dev araçları gizli ama erişilebilir.
