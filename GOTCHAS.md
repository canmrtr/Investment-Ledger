...
- **`p.currency` vs price_cache (priceCur kuralı)**: `prc[ticker]` BIST/BES→TRY, CASH/DEPOSIT→pozisyon currency'si (synthetic, default TRY), diğer→USD. MV hesabında priceCur'u **daima** şöyle yaz: `(p.type==="BIST"||p.type==="BES")?"TRY":(p.type==="CASH"||p.type==="DEPOSIT")?(p.currency||"TRY"):"USD"`. Kısayol `p.type==="BIST"?"TRY":"USD"` **yanlış** — TL mevduatı USD sayıp USDTRY kuru kadar (~38x) şişirir (2026-06-04 AnalysisTab `mvDisp`/`priceCurOf` bug'ı). App.js `wrapPos`/`allDisp` referans alınmalı. Cost için `p.currency` doğru.
