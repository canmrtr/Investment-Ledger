...
- **`p.currency` vs price_cache (priceCur kuralı) — TRAP**: MV/para-değer hesabında `price_cache`'in para birimini tahmin ederken kısayol `p.type==="BIST"?"TRY":"USD"` **yanlış** — TL mevduatı (CASH/DEPOSIT) USD sayıp USDTRY kuru kadar (~38x) şişirir (2026-06-04 AnalysisTab `mvDisp`/`priceCurOf` bug'ı; bkz. `Lessons.md`). Canonical `priceCur` formülünü **`FEATURE_DETAILS.md` "Currency Handling"**'den kopyala — buraya tekrar yazma. App.js `wrapPos`/`allDisp` referans implementasyon. Cost için `p.currency` doğru.
...
- **localStorage scope**: user-scoped key'ler **her** signOut handler'ında (Settings + hamburger) temizlenmeli — biri eksikse cross-user cache leak. `fund_*/meta_*/sec_ticker_db` public data, user-bound değil. Tam key/TTL/scope manifesti → `CACHE.md`.
...
- **TEFAS NAV (`fonFiyatBilgiGetir`)**: `{fonKodu, periyod}` (periyod = ay sayısı: 1/3/6/12) POST eder; `resultList` **eski→yeni** sıralı (`fiyat` JSON number, `tarih` `YYYY-MM-DD`). `periyod=1` ~14-22 işlem günü döner — tatil loop'u gereksiz. Legacy `/api/DB/BindHistoryInfo` 2026-04'te kaldırıldı. UA + Referer/Origin header zorunlu (WAF). `fiyat` defansif `String(x).replace(",",".")` ile parse edilir.
- **⚠️ TEFAS bugünün NAV'ı = 0**: `resultList`'in SON elemanı (bugün) NAV akşam yayınlanana kadar `fiyat:0` döner. Körlemesine son elemanı alma — **sondan başlayıp `fiyat>0` olan son yayınlanmış NAV'ı al** (`tefasLastPublished`/`fetchTefasPrice` loop'u). Aksi halde gün içi tüm TEFAS fiyatları 0 yazılır (NaN guard 0'ı yakalamaz). Canlı doğrulamada (2026-06-05) yakalandı.
- **TEFAS price_cache yazımı**: `price_cache`'de **`source`/`currency` kolonu YOK**. İki ayrı yazıcı var: (1) `fetch-prices` TEFAS **price mode** yalnız `{ticker, price, updated_at}` yazar (kullanıcı sayfada gezerken anlık NAV); (2) **`refresh-price-cache` cron (6h)** `fetchTefasHistorical` `periyod=12` ile d1/w1/m1/y1 + p_d1…p_y1 + h_52w/l_52w'i de doldurur (Sprint 26 — US/BIST ile aynı şema). Yani delta'lar başta NULL, ilk cron run'dan sonra dolu. Currency read-time'da `positions.currency='TRY'` ile implied.
