...
**Kod fazı (2026-06-04):** 9/9 alt-task tek oturumda tamamlandı (executing-plans skill). Plandan 2 bilinçli sapma: (a) `tefas-catalog` modu **JWT arkasında** bırakıldı (plan `skipJwt` öneriyordu) — anon kotа suistimalini önlemek için, Sprint 15 auth-hardening paritesi; (b) SearchTab `tefas_funds` fetch'i **5×1000 sayfalandı** (plan tek `.select()` idi, PostgREST 1k limiti ~3510 fonun çoğunu keserdi). 3 edge fn `edge-reviewer`'dan geçti; fetch-prices'ta HIGH/MEDIUM bulgular (AbortSignal.timeout, NaN guard, ticker.toUpperCase) düzeltildi; tüm TEFAS fetch'lerine Referer/Origin (WAF dayanıklılığı) + defansif decimal normalizasyon eklendi. `tefas_funds` RLS `rls-auditor`'dan PASS. babel/edge/drift yeşil.

**Plan File Map'inde olmayıp doğrulama (Task 12) sırasında yakalanan entegrasyon gap'leri** (3 ek commit): (a) **ManuelPosForm** — TEFAS currency-default listesinde ve type `<select>`'inde yoktu → USD'ye düşüyor + boş tip; TRY default + "Fon Kodu"/"YAC" placeholder eklendi. (b) **App.js** — 2× `priceCur` + Dashboard `toUsd` TEFAS'ı USD sayıyordu (yanlış MV); `fetchPrices` filtresi TEFAS'ı dışlıyordu (manuel refresh NAV çekmiyordu) → hepsi düzeltildi; `fetchHist` bilinçli TEFAS'sız (sparkline out-of-scope). (c) **TickerDetailTab** — discovery-mode currency USD'ye düşüyordu + TEFAS için alakasız US temettü-takvimi fetch'i; TRY default + divCal skip (supportsFund zaten TEFAS'ı dışlıyor, fundamentals fetch yok). loadData `price_cache.select("*")` ile tüm ticker'ları okuduğundan cron-populated TEFAS NAV otomatik akıyor.

**Canlı deploy + doğrulama (2026-06-05):** 3 edge fn deploy edildi (CLI); migration Supabase MCP `apply_migration` ile uygulandı (CLI history bozuk olduğu için `db push` kullanılamadı); katalog authenticated edge çağrısıyla yüklendi → **3509 fon, 0 hata** (`tefas_funds` AAJ→ZZL, hepsi kategorili). NAV smoke test'te **canlı bug yakalandı**: `fonFiyatBilgiGetir` `resultList`'in son elemanı (bugün) NAV akşam yayınlanana kadar `fiyat:0` döndürüyor → körlemesine son elemanı almak gün-içi 0 fiyat yazıyordu (YAC=0). Fix: `tefasLastPublished` — sondan başlayıp `fiyat>0` olan son yayınlanmış NAV'ı al (fetch-prices + refresh-price-cache); yeniden deploy + retest → YAC=14.05 ✓. GOTCHAS'a eklendi. **Frontend zaten canlıda (push edildi); özellik artık tam aktif.**
